...
| Lock de archivo entre isolates alrededor de `AppDatabase.init()` (D-34) | Encontrado en el corte real: WorkManager corrió `init()` en paralelo al isolate principal en el primer arranque, `sqflite` tiró una excepción cruda que activó el camino de "BD corrupta" sin corrupción real. Riesgo estructural, no solo del día del corte — cualquier tick de background durante un arranque en frío puede repetirlo | ✓ Good — datos originales confirmados intactos, fix con test antes de reintentar en el celular |
